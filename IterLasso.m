...
% the loop stops when t-test insignificant twice
STOPPING_COUNTER = 0;
STOPPING_RULE = 2;  
chance = 2/3 + 1e-4;
...
while true
    ...
    for i = 1:k    
        ...
        % Test set
        FINAL_HOLDOUT = CVBLOCKS(:,i);

        % CV Indices for cvglmnet
        CV2 = CVBLOCKS(~FINAL_HOLDOUT,(1:k)~=i); 
       ...
       % Subset the data, choose only used voxels
        ...
        test.prediction(:,i) = (Xtest * fitObj.beta + repmat(fitObj.a0, [test.size, 1])) > 0 ;
        test.accuracy(:,i) = mean(Ytest == test.prediction(:,i))';
        
        % Releveling
        opts.alpha = 0;
        fitObj_ridge = glmnet(Xtrain, Ytrain, 'binomial', opts);
        r.prediction(:,i) = (Xtest * fitObj_ridge.beta + repmat(fitObj_ridge.a0, [test.size, 1])) > 0 ;  
        r.accuracy(:,i) = mean(Ytest == r.prediction(:,i))';
        

        % Keeping track of which set of voxels were used in each cv block
        used( i, ~used(i,:) ) = fitObj.beta ~= 0;
...
for i = 1:k
    ...
    Xfinal = X( :, USED{numIter - STOPPING_RULE}(i,:) );

    % Split the final data set to testing set and training set 
    Xtest = Xfinal(CVBLOCKS(:,i) ,:);
    Xtrain = Xfinal(~CVBLOCKS(:,i) ,:);
    ...
    final.prediction(:,i) = (Xtest * fitObj_Final.beta + repmat(fitObj_Final.a0, [test.size, 1])) > 0 ;  
    final.accuracy(i) = mean(Ytest == final.prediction(:,i))';
